...
def mostrar_predicciones(predicciones, reales, modelo):
    print(f"\nModelo: {modelo}")
    for i in range(10):
        print(f"Salida real: {reales[i]}, Predicción: {predicciones[i]}")
...
# Entrenar modelos
model_seguros.fit(x_entrenamiento_seguro, y_entrenamiento_seguro, epochs=100, batch_size=32, verbose=0)
model_vino.fit(x_entrenamiento_vino, y_entrenamiento_vino, epochs=100, batch_size=32, verbose=0)
model_diabetes.fit(x_entrenamiento_diabetes, y_entrenamiento_diabetes, epochs=500, verbose=0)
...
# Evaluar los modelos de Máquinas de Vector Soporte
...
# Evaluar modelos de Naive Bayes
